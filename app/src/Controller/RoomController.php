...
class RoomController extends AbstractController
{
    ...
    #[Route('/lies', name: 'app_homev')]
    public function index87(): Response
    {
    ...
    public function join(Request $request, HubInterface $hub, RoomRepository $roomRepository, EntityManagerInterface $em, PlayerRepository $playerRepository, GlobalState $globalState)
    {
        ...
        if ($numberOfActiveParticipant == $room->getNumberParticipant()) {
            ...
            $this->publishToWebSocket("/" . $room->getId() . "/ready", ['test' => 'This is a test ready room ']);
    ...
    public function Realgame($id, $player_token, TableRepository $tableRepository, TimeRepository $timeres, RoomRepository $roomRepository, CardsRepository $cardsRepository, PlayerRepository $playerRepository, HandRepository $handRepository, RoundRepository $roundRepository, EntityManagerInterface $em): Response
    {
        ...
        $players = $playerRepository->findBy(["room" => $room]);
        // Assuming $players is an array or collection of player objects
        ...
        $verifPlayers = $playerRepository->findBy(["room" => $room]);
        //dd($verifPlayers);
        $winningPlayerIds = [];
        foreach ($verifPlayers as $element) {
            $winner = $element->getWinner();
            if ($winner == "1") {
                $winningPlayerIds[] = $element->getId();
            }
            // dd($player->getId(), $winner); // Check each player's ID and winner status
        }
    ...
    public function put(
        ...
        try {
            ...
            if (count($rounds) > 0) {
                ...
                $idOfWinnigPlyaer = $winnerPlayer->getId();
                if (count($handwinner) == 0) {
                    ...
                    //winig player verificaton ----------------------------------------------------------------------------------
                    $em->getConnection()->beginTransaction(); // Start transaction
                    try {
                        $winnerPlayer = $playerRepository->find($idOfWinnigPlyaer);
                       // dd($winnerPlayer);
                       $playerRepository->updateWinnerWithQueryBuilder($winnerPlayer->getId(),$nb+1);
                        /*$winnerPlayer->setWinner(1);
                        $winnerPlayer->setRank($nb + 1);
                        $em->persist(object: $winnerPlayer);
                        $em->persist(object: $winnerPlayer);
                        $em->flush();
                        $em->flush();*/
                        $em->getConnection()->commit(); // Commit transaction
                    } catch (\Exception $e) {
                        $em->getConnection()->rollBack(); // Rollback transaction on failure
                        throw $e;
                    }
                    //-----------------------------------------------------------------------
            ...
            $NumberOfWinPlyers = $playerRepository->countWinnersInRoom($room->getId());
            //  dd($NumberOfWinPlyers);

          /*  if ($NumberOfWinPlyers == $room->getNumberParticipant() - 1) {
                return new JsonResponse(['Game state ' => 'Game is over'], Response::HTTP_NOT_FOUND);
...
*/
            ...
            $round->setBlock(0);
            $em->persist($round);
            $playert = $playerRepository->find($id);
            foreach ($cards as $card) {
                $selectedCardT = $handRepository->findOneBy(["player" => $playert, "card" => $card]);
                if ($selectedCardT) {
                    if ($selectedCardT) {
                        $em->remove($selectedCardT);
                        $em->flush();
                        $table = new Table();
                        $table->setCards(cards: $card);
                        $table->setRound(round: $round);
                        $em->persist(object: $table);
                    } else {
            ...
            $this->publishToWebSocket('/' . $room->getId() . '/putdone', [
                'update' => 'put done ' . date("h:i:sa"),
                'currentroundplayer' => $player->getId(),
                'nextplayer' => $nextPlayer->getId(),
                'cardnumber' => $cardCount
    ...
    public function lie($id, Request $request, HubInterface $hubInterface, TableRepository $tableRepository, HandRepository $handRepository, RoomRepository $roomRepository, PlayerRepository $playerRepository, RoundRepository $roundRepository, EntityManagerInterface $em)
    {
        ...
        if (count($rounds) == 0) {
            return new JsonResponse(["message" => "there is no rounds because its  has been taken by " . $id, Response::HTTP_UNAUTHORIZED]);
        }
        ...
        if ($desicion == 1) {
            ...
            foreach ($players as $pl) {
                ...
                //illi kadeb  (illi ras lie) ou talaa 3andou il 7a9
                $p;
                //illi l3ab 9ablou  (illi l3ab tour illi 9ablou )
                $roundplayer;

                $this->publishToWebSocket("/win-loser-players-broadcast", ['right' => $p->getId(), "wrong" => $roundplayer->getId()]);

            }
            $em->flush();
            #mercure upadate he will be playing
            #give the enemy all the cards 
            //the first verifucation 
            $nbround = 0;
            ...
            #send update to the loser player
        ...
        # in case he was wrong he will take all the cards and the enemy status updated
            ...
            /*  $update1 = new Update(
                  "/broadcastAngel" ,
                  json_encode(['broadcastangel' =>$angel])
              );
              $hubInterface->publish($update1);
  */
    ...
    private function publishToWebSocket($topic, $data)
    {
        $msg = json_encode(['topic' => $topic, 'data' => $data]);

        // Log the message to make sure it's being created
        error_log("Publishing message to WebSocket: " . $msg);
        ...
        $connector('ws://3.68.191.127:1700')
            ->then(function (WebSocket $conn) use ($msg) {
                ...
                // Log if there's an error in connecting
                error_log("Could not connect: " . $e->getMessage());
            });

        // Run the loop to keep the connection active
        $loop->run();
    }
